...
std::string write_tree(const std::string &dest) {
  ...
  for (const auto &entry : std::filesystem::directory_iterator(dest)) {
    ...
    std::string mode =
        std::filesystem::is_directory(entry.status()) ? "40000" : "100644";
    std::string sha1_hash = std::filesystem::is_directory(entry.status())
                                ? write_tree(full_path)
                                : create_blob(full_path);

    if (!sha1_hash.empty()) {
      entries.emplace_back(name, std::string{mode} + " " + name + '\0' +
                                     hex_to_binary(sha1_hash));
    }
  }
  ...
  std::string obj_path =
      "./.git/objects/" + tree_sha1.substr(0, 2) + "/" + tree_sha1.substr(2);

  if (system(("mkdir -p " + obj_path.substr(0, obj_path.find_last_of('/')))
                 .c_str()) != 0) {
...
void restore_tree(const std::string &hash, const std::string &dest,
  ...
  std::string object_path =
      repo_dir + "/.git/objects/" + hash.substr(0, 2) + '/' + hash.substr(2);
