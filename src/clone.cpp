...
std::string apply_delta(const std::string &delta, const std::string &base) {
  ...
  while (pos < delta.size()) {
    ...
    if (inst & 0x80) {
      ...
      for (int i = 3; i >= 0; i--)
        if (inst & (1 << i))
          offset = (offset << 8) | static_cast<unsigned char>(delta[pos++]),
          offsetBytes++;
      for (int i = 6; i >= 4; i--)
        if (inst & (1 << i))
          size = (size << 8) | static_cast<unsigned char>(delta[pos++]),
          sizeBytes++;
  ...
  for (int i = 0; i < num_objects; ++i) {
    ...
    if (type == 6) {
      ...
      std::string object_type =
          base_contents.substr(0, base_contents.find(' '));
      base_contents = base_contents.substr(base_contents.find('\0') + 1);
      std::string delta = decompress_str(pack.substr(pos));
      std::string reconstructed = object_type + ' ' +
                                  std::to_string(base_contents.length()) +
                                  '\0' + apply_delta(delta, base_contents);
      ...
      if (hash == packhash)
        master_commit_contents = reconstructed.substr(reconstructed.find('\0'));
    } else {
      std::string object_contents = decompress_str(pack.substr(pos));
      pos += compress_str(object_contents).length();
      std::string object_type_str = (type == 1)   ? "commit "
                                    : (type == 2) ? "tree "
                                                  : "blob ";
